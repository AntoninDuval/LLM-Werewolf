...
class GameState:
    ...
    def kill_player(self, player):
        ...
        if player in self.alive_players:
            ...
            player.alive = False
            self.chat.add_message_p2p(
                "Game Master", f"{player.name} was a {player.role} !"
            )
        else:
    ...
    def check_victory(self):
        ...
        if werewolf_count == 0:
            self.chat.add_message_p2p(
                "Game Master", "Werevolves are gone ! Villagers have won the game!"
            )
            self.winner = "Villagers"
            self.game_over = True
        elif werewolf_count >= villager_count:
            self.chat.add_message_p2p(
                "Game Master", "Werewolves have taken over the village!"
            )
